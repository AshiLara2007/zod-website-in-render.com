...
export default function Home() {
  ...
  if (!languageSelected) {
    return (
      <div dir={dir} className="fixed inset-0 bg-gradient-to-br from-[#0a0a2a] via-[#1a1a4a] to-[#0a0a2a] z-[200] flex flex-col items-center justify-center">
        <div className="text-center px-4">
          <div className="relative">
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 blur-2xl animate-pulse"></div>
            <div className="absolute inset-0 rounded-full bg-white/10 blur-xl"></div>
            <img src="/logo/logo.jpeg" alt="ZOD MANPOWER" className="w-20 h-20 md:w-24 md:h-24 rounded-full mx-auto mb-6 md:mb-8 object-cover shadow-2xl relative z-10 animate-float border-2 border-cyan-400/50" onError={(e) => (e.currentTarget.style.display = 'none')} />
          </div>
          <h1 className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent mb-3 md:mb-4 animate-bounce">{language === 'en' ? 'Welcome To Doha' : 'مرحباً بكم في الدوحة'}</h1>
          <p className="text-lg md:text-2xl text-cyan-200/80 mb-6 md:mb-8">{t.brandLoading}</p>
          <div className="flex gap-3 md:gap-4 justify-center flex-wrap">
            <button onClick={() => { setLanguage('en'); setLanguageSelected(true); }} className="px-6 md:px-8 py-2.5 md:py-3 rounded-full font-bold transition-all duration-300 transform hover:scale-110 bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-xl hover:shadow-2xl">English</button>
            <button onClick={() => { setLanguage('ar'); setLanguageSelected(true); }} className="px-6 md:px-8 py-2.5 md:py-3 rounded-full font-bold transition-all duration-300 transform hover:scale-110 bg-gradient-to-r from-cyan-500 to-blue-500 text-white shadow-xl hover:shadow-2xl">العربية</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div dir={dir} className={isRTL ? 'rtl' : 'ltr'}>
      <style>{`
        ...
        @keyframes glowPulse { 0%, 100% { text-shadow: 0 0 5px rgba(0,255,255,0.3); box-shadow: 0 0 5px rgba(0,255,255,0.2); } 50% { text-shadow: 0 0 20px rgba(0,255,255,0.6); box-shadow: 0 0 20px rgba(0,255,255,0.4); } }
        .glow-pulse { animation: glowPulse 2s infinite; }
        @keyframes borderFlow { 0% { border-color: rgba(0,255,255,0.3); } 50% { border-color: rgba(0,100,255,0.8); } 100% { border-color: rgba(0,255,255,0.3); } }
        .border-flow { animation: borderFlow 3s infinite; }
        .reveal { opacity: 0; transform: translateY(30px); transition: all 0.8s ease-out; }
        .reveal.active { opacity: 1; transform: translateY(0); }
        .glass-nav { background: rgba(10, 10, 42, 0.85); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(0,255,255,0.2); box-shadow: 0 4px 20px rgba(0,0,0,0.3); }
        .nav-link { position: relative; }
        .nav-link::after { content: ''; position: absolute; bottom: -5px; left: 0; width: 0; height: 2px; background: linear-gradient(90deg, #00ffff, #0066ff); transition: width 0.3s ease; }
        .nav-link:hover::after { width: 100%; }
        .qatar-gradient { background: linear-gradient(135deg, #0a0a2a 0%, #1a1a4a 50%, #0a0a2a 100%); }
        .mobile-sidebar { position: fixed; top: 0; right: -280px; width: 280px; height: 100vh; background: rgba(10,10,42,0.95); backdrop-filter: blur(12px); z-index: 1000; transition: right 0.3s ease; box-shadow: -2px 0 20px rgba(0,255,255,0.1); padding: 20px; overflow-y: auto; border-left: 1px solid rgba(0,255,255,0.2); }
        .mobile-sidebar.active { right: 0; }
        .sidebar-close { position: absolute; top: 20px; right: 20px; font-size: 24px; cursor: pointer; color: #00ffff; }
        .sidebar-nav { display: flex; flex-direction: column; gap: 20px; margin-top: 60px; }
        .sidebar-nav a, .sidebar-nav button { font-size: 16px; font-weight: 600; text-transform: uppercase; color: #fff; padding: 10px 0; border-bottom: 1px solid rgba(0,255,255,0.2); transition: all 0.3s; }
        .sidebar-nav a:hover, .sidebar-nav button:hover { color: #00ffff; transform: translateX(5px); }
        .sidebar-apply { background: linear-gradient(90deg, #00ffff, #0066ff); color: #0a0a2a; padding: 12px; border-radius: 30px; text-align: center; margin-top: 20px; font-weight: bold; }
        ...
        .web3-card { background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); border: 1px solid rgba(0,255,255,0.2); border-radius: 1.5rem; transition: all 0.3s ease; }
        .web3-card:hover { background: rgba(255,255,255,0.1); border-color: #00ffff; transform: translateY(-5px); box-shadow: 0 20px 40px rgba(0,0,0,0.3), 0 0 20px rgba(0,255,255,0.2); }
        .gradient-border { position: relative; background: linear-gradient(135deg, #00ffff, #0066ff, #00ffff); background-size: 200% 200%; animation: gradientShift 3s ease infinite; }
      ...
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-[100] flex flex-col items-end gap-3">
        {chatOpen && (
          <div className="w-72 sm:w-80 md:w-96 bg-[#0a0a2a]/90 backdrop-blur-xl rounded-[1.5rem] md:rounded-[2rem] shadow-2xl border border-cyan-500/30 flex flex-col overflow-hidden animate-fade-in-up">
            <div className="bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-3 md:px-5 md:py-4 flex items-center justify-between">
              ...
              {!chatLanguageSelected ? (
                <div className="text-center py-6 md:py-8 space-y-3 md:space-y-4">
                  ...
                  <div className="flex gap-2 md:gap-3 justify-center">
                    <button onClick={() => startChat('en')} className="px-4 md:px-5 py-1.5 md:py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full text-xs md:text-sm font-bold hover:scale-105 transition">English</button>
                    <button onClick={() => startChat('ar')} className="px-4 md:px-5 py-1.5 md:py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full text-xs md:text-sm font-bold hover:scale-105 transition">العربية</button>
                  </div>
                </div>
              ) : (
                <>
                  {chatMessages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? (isRTL ? 'justify-start' : 'justify-end') : (isRTL ? 'justify-end' : 'justify-start')}`}>
                      <div className={`max-w-[85%] px-3 py-2 md:px-4 md:py-3 rounded-xl md:rounded-2xl text-xs md:text-sm leading-relaxed shadow-sm whitespace-pre-wrap ${msg.role === 'user' ? 'bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-br-sm' : 'bg-gray-800 text-gray-200 rounded-bl-sm border border-cyan-500/30'}`}>{msg.text}</div>
                    </div>
                  ))}
                  {chatLoading && (
                    <div className="flex justify-start">
                      <div className="bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl px-3 py-2 md:px-4 md:py-3 rounded-bl-sm shadow-sm flex gap-1 items-center">
                        <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></span>
                        <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></span>
                        <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
                      </div>
                    </div>
                  )}
                </>
            ...
            {chatLanguageSelected && (
              <div className="px-3 py-2 md:px-4 md:py-3 border-t border-cyan-500/30 bg-gray-900 flex gap-2">
                <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={handleChatKey} placeholder={chatLanguageSelected === 'en' ? "Ask me anything..." : "اسألني أي شيء..."} className="flex-1 px-3 py-2 md:px-4 md:py-2.5 bg-gray-800 border border-cyan-500/30 rounded-xl text-xs md:text-sm outline-none focus:border-cyan-500 text-white transition-all" disabled={chatLoading} />
                <button onClick={sendChatMessage} disabled={chatLoading} className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-xl flex items-center justify-center hover:scale-105 transition-all active:scale-95 disabled:opacity-50"><i className="fa-solid fa-paper-plane text-xs"></i></button>
              </div>
            )}
          </div>
        )}
        <button onClick={() => setChatOpen(!chatOpen)} className="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all duration-300 hover:shadow-[0_0_25px_rgba(0,255,255,0.5)] active:scale-95">{chatOpen ? <i className="fa-solid fa-xmark text-lg md:text-xl"></i> : <i className="fa-regular fa-message text-lg md:text-xl"></i>}</button>
      </div>

      {loginModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center backdrop-blur-sm p-4 animate-fade-in-up">
          <div className="bg-[#0a0a2a] p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] w-full max-w-md shadow-2xl relative border border-cyan-500/30">
            <button onClick={() => setLoginModalOpen(false)} className="absolute top-4 right-4 md:top-6 md:right-6 text-gray-400 hover:text-cyan-400 transition-transform hover:rotate-90"><i className="fa-solid fa-circle-xmark text-xl md:text-2xl"></i></button>
            <div className="text-center mb-6 md:mb-8">
              <i className="fa-solid fa-user-shield text-4xl md:text-5xl text-cyan-400 mb-3 md:mb-4"></i>
            ...
            <form onSubmit={handleLogin} className="space-y-4 md:space-y-5" autoComplete="off">
              <div><label className="text-[10px] md:text-xs font-bold uppercase text-cyan-400 ml-1">{t.username}</label><input type="text" id="adminUser" placeholder={t.enterAdmin} autoComplete="off" className="w-full p-3 md:p-4 bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-cyan-500 text-white transition-all" required /></div>
              <div><label className="text-[10px] md:text-xs font-bold uppercase text-cyan-400 ml-1">{t.password}</label><input type="password" id="adminPass" placeholder="••••••••" autoComplete="new-password" className="w-full p-3 md:p-4 bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-cyan-500 text-white transition-all" required /></div>
              <button type="submit" className="w-full py-3 md:py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold rounded-xl md:rounded-2xl hover:shadow-lg transition-all shadow-lg">{t.authorizedOnly}</button>
            </form>
          </div>
        </div>
      )}
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 bg-black/70 z-[150] flex items-center justify-center backdrop-blur-sm p-4 animate-fade-in-up">
          <div className="bg-[#0a0a2a] p-6 md:p-8 rounded-xl md:rounded-2xl max-w-md w-full shadow-2xl border border-cyan-500/30">
      ...
      {!adminActive ? (
        <div className="public-section">
          <div className="relative overflow-hidden bg-gradient-to-r from-cyan-600 via-blue-600 to-cyan-600 pt-20 md:pt-24 pb-2 md:pb-3 px-4 md:px-6 gradient-border">
          ...
          <nav className="fixed w-full z-50 glass-nav" style={{ top: 0 }}>
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex justify-between items-center">
              <div className="flex items-center space-x-2 md:space-x-3 cursor-pointer group" onClick={() => { setShowHirePage(false); setShowOurTeamPage(false); setShowAboutPage(false); window.scrollTo(0, 0); }}>
                <img src="/logo/logo.jpeg" alt="ZOD MANPOWER" className="w-8 h-8 md:w-12 md:h-12 rounded-xl object-cover shadow-lg transition-transform duration-300 group-hover:scale-110 border border-cyan-400/30" />
                <div className="text-sm md:text-2xl font-extrabold tracking-tighter uppercase bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{t.brandName}</div>
              </div>
              <div className="hidden lg:flex items-center space-x-4 md:space-x-8 font-semibold text-[10px] md:text-xs uppercase tracking-widest">
                <a href="#home" onClick={() => { setShowHirePage(false); setShowOurTeamPage(false); setShowAboutPage(false); }} className="nav-link text-white hover:text-cyan-400 transition-all duration-300">{t.home}</a>
                <a href="#about" onClick={() => { setShowHirePage(false); setShowOurTeamPage(false); setShowAboutPage(false); }} className="nav-link text-white hover:text-cyan-400 transition-all duration-300">{t.about}</a>
                <a href="#services" onClick={() => { setShowHirePage(false); setShowOurTeamPage(false); setShowAboutPage(false); }} className="nav-link text-white hover:text-cyan-400 transition-all duration-300">{t.services}</a>
                <button onClick={() => { setShowOurTeamPage(true); setShowHirePage(false); setShowAboutPage(false); }} className="nav-link text-white hover:text-cyan-400 transition-all duration-300">{t.ourTeam}</button>
                <button onClick={() => { setShowHirePage(true); setShowOurTeamPage(false); setShowAboutPage(false); }} className="nav-link text-white hover:text-cyan-400 transition-all duration-300">{t.hireNav}</button>
                <a href="https://wa.me/97455355206" onClick={() => trackLead('Nav Apply', 'Global Apply')} target="_blank" className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-4 md:px-6 py-2 md:py-2.5 rounded-full shadow-md hover:shadow-lg transition-all hover:scale-105 active:scale-95 text-[10px] md:text-xs">{t.contactUs}</a>
                <button onClick={() => setLanguage(language === 'en' ? 'ar' : 'en')} className="flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold bg-gray-800 text-cyan-400 hover:bg-gray-700 transition-all border border-cyan-500/30"><i className="fa-solid fa-globe text-[8px] md:text-[10px]"></i><span>{language === 'en' ? 'العربية' : 'English'}</span></button>
              </div>
              <button className="lg:hidden text-xl md:text-2xl text-cyan-400" onClick={() => setSidebarOpen(true)}><i className="fa-solid fa-bars-staggered"></i></button>
            </div>
          </nav>
          ...
          <div className={`mobile-sidebar ${sidebarOpen ? 'active' : ''}`} style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
            ...
            <div className="sidebar-nav mt-8">
              ...
              <button onClick={() => { setLanguage(language === 'en' ? 'ar' : 'en'); setSidebarOpen(false); }} className="mt-4 w-full py-2 bg-gray-800 rounded-full text-sm font-bold text-cyan-400 hover:bg-gray-700 transition-all flex items-center justify-center gap-2 border border-cyan-500/30"><i className="fa-solid fa-globe text-xs"></i>{language === 'en' ? 'العربية' : 'English'}</button>
            </div>
          </div>
          <div className={`sidebar-overlay ${sidebarOpen ? 'active' : ''}`} onClick={() => setSidebarOpen(false)}></div>

          {showOurTeamPage ? (
            <div className="min-h-screen pt-24 md:pt-32 pb-16 md:pb-20 px-4 md:px-6 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a]">
              <div className="max-w-7xl mx-auto">
                <button onClick={() => setShowOurTeamPage(false)} className="flex items-center gap-2 text-cyan-400 font-bold text-xs md:text-sm mb-6 md:mb-8 hover:underline transition-all"><i className="fa-solid fa-arrow-left"></i> {t.backToHome}</button>
                <div className="text-center mb-8 md:mb-12 reveal">
                  <h3 className="text-2xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent mb-3 md:mb-4">{t.teamTitle}</h3>
                  <p className="text-gray-400 text-sm md:text-base max-w-2xl mx-auto">{t.teamDesc}</p>
                </div>
                <div className="mb-12 md:mb-16">
                  <h4 className="text-xl md:text-2xl font-bold text-cyan-400 text-center mb-6 md:mb-10">{t.topManagementTitle}</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                    {topManagementTeam.map((member) => (
                      <div key={member.id} className="bg-[#0a0a2a]/50 backdrop-blur-sm rounded-xl md:rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border border-cyan-500/30 group">
                        <div className="relative h-48 md:h-64 bg-gradient-to-r from-cyan-500 to-blue-600 flex items-center justify-center">
                          <img src={member.photo} alt={member.name} className="w-28 h-28 md:w-36 md:h-36 rounded-full object-cover border-4 border-cyan-400 shadow-lg transition-transform duration-300 group-hover:scale-105" onError={(e) => (e.currentTarget.src = 'https://placehold.co/150x150?text=User')} />
                ...
                <div>
                  <h4 className="text-lg md:text-xl font-bold text-cyan-400 text-center mb-6 md:mb-8">Our Dedicated Team</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {regularTeam.map((member) => (
                      <div key={member.id} className="bg-[#0a0a2a]/50 backdrop-blur-sm rounded-lg md:rounded-xl p-4 md:p-5 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 flex items-center gap-3 md:gap-4 border border-cyan-500/30">
                        <img src={member.photo} alt={member.name} className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover border-2 border-cyan-400/50" onError={(e) => (e.currentTarget.src = 'https://placehold.co/80x80?text=User')} />
                        <div className="flex-1">
                          <h5 className="font-bold text-white text-sm md:text-base">{escapeHtml(member.name)}</h5>
                          <p className="text-cyan-400 text-[10px] md:text-xs font-semibold">{escapeHtml(member.position)}</p>
                        </div>
            ...
            <div className="min-h-screen pt-24 md:pt-32 pb-16 md:pb-20 px-4 md:px-6 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a]">
              <div className="max-w-7xl mx-auto">
                <button onClick={() => setShowAboutPage(false)} className="flex items-center gap-2 text-cyan-400 font-bold text-xs md:text-sm mb-6 md:mb-8 hover:underline transition-all"><i className="fa-solid fa-arrow-left"></i> {t.backToHome}</button>
                <div className="bg-[#0a0a2a]/50 backdrop-blur-sm rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-lg border border-cyan-500/30">
                  <div className="grid md:grid-cols-2 gap-8 md:gap-12">
                    <div><h3 className="text-2xl md:text-3xl font-bold text-cyan-400 mb-4 md:mb-6">{t.ourVision}</h3><p className="text-gray-300 leading-relaxed text-sm md:text-lg">{t.visionText}</p></div>
                    <div><h3 className="text-2xl md:text-3xl font-bold text-cyan-400 mb-4 md:mb-6">{t.ourMission}</h3><p className="text-gray-300 leading-relaxed text-sm md:text-lg">{t.missionText}</p></div>
                  </div>
                  <div className="mt-8 md:mt-12 pt-6 md:pt-8 border-t border-cyan-500/30">
            ...
            <div className="min-h-screen pt-24 md:pt-32 pb-16 md:pb-20 px-4 md:px-6 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a]">
              <div className="max-w-7xl mx-auto">
                <button onClick={() => router.back()} className="flex items-center gap-2 text-cyan-400 font-bold text-xs md:text-sm mb-6 md:mb-8 hover:underline transition-all"><i className="fa-solid fa-arrow-left"></i> {t.backToHome}</button>
                <div className="flex flex-col md:flex-row justify-between items-end mb-6 md:mb-10 gap-4 md:gap-6">
                  <div><h3 className="text-2xl md:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent mb-2">{t.hireTitle}</h3><p className="text-gray-400 text-sm md:text-base">{t.hireDesc}</p></div>
                  <div className="flex gap-2 md:gap-3 w-full md:w-auto flex-wrap">
                    <div className="relative flex-1 min-w-[150px] md:min-w-[180px]">
                      <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder={t.searchPlaceholder} className="w-full p-3 md:p-4 pl-8 md:pl-12 bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-cyan-500 text-white transition-all text-sm md:text-base" />
                      <i className="fa-solid fa-magnifying-glass absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-cyan-400 text-xs md:text-sm"></i>
                    </div>
                    <select value={countryFilter} onChange={(e) => setCountryFilter(e.target.value)} className="p-3 md:p-4 bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl outline-none focus:ring-2 focus:ring-cyan-500 transition-all text-xs md:text-sm font-bold text-white">
                      <option value="">{t.allCountries}</option>
                      {countryOptions.map((c) => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <label className="flex items-center gap-2 px-3 py-2 bg-gray-800 border border-cyan-500/30 rounded-xl cursor-pointer hover:bg-gray-700 transition-all">
                      <input type="checkbox" checked={showReturnedOnly} onChange={(e) => setShowReturnedOnly(e.target.checked)} className="w-4 h-4 text-cyan-500 rounded" />
                      <span className="text-xs font-medium text-gray-300 whitespace-nowrap">{t.showReturnedOnly}</span>
                    </label>
                    <button onClick={fetchTalents} className="px-4 md:px-5 py-3 md:py-4 bg-gray-800 border border-cyan-500/30 rounded-xl md:rounded-2xl hover:bg-gray-700 transition-all hover:scale-105 text-cyan-400" title={t.refresh}><i className="fa-solid fa-rotate-right text-xs md:text-sm"></i></button>
                  </div>
                </div>
                {loading ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">{[...Array(6)].map((_, i) => <div key={i} className="bg-gray-800/50 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 animate-pulse"><div className="w-16 h-16 md:w-20 md:h-20 bg-gray-700 rounded-2xl mb-4"></div><div className="h-5 bg-gray-700 rounded w-3/4 mb-2"></div><div className="h-4 bg-gray-700 rounded w-1/2"></div></div>)}</div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {filteredTalents.map((talent) => (
                      <div key={talent.id} className="web3-card bg-gray-800/30 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-2 flex flex-col h-full">
                        <div className="flex justify-between items-start mb-4 md:mb-6">
                          <img src={talent.pic} className="w-16 h-16 md:w-20 md:h-20 rounded-2xl object-cover border-2 border-cyan-400/30 shadow-sm" onError={(e) => (e.currentTarget.src = 'https://placehold.co/100x100?text=User')} alt={talent.name} />
                        ...
                        <div className="flex-grow">
                          <Link href={`/candidate/${talent.id}`}><h4 className="font-bold text-white text-lg md:text-xl leading-tight hover:text-cyan-400 cursor-pointer transition-colors">{escapeHtml(talent.name)}</h4></Link>
                          <p className="text-cyan-400 font-bold text-[10px] md:text-[11px] uppercase tracking-widest mt-1">{escapeHtml(talent.job)}</p>
                          <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-cyan-500/30 space-y-2 md:space-y-3 mb-6 md:mb-8">
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-earth-asia w-4 md:w-5 text-cyan-400"></i><span>{escapeHtml(talent.country)}</span></div>
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-user w-4 md:w-5 text-cyan-400"></i><span>{talent.gender}, {talent.age} Years</span></div>
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-money-bill-wave w-4 md:w-5 text-cyan-400"></i><span>{talent.salary || 0} QAR</span></div>
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-calendar-alt w-4 md:w-5 text-cyan-400"></i><span>{talent.experience || '2-5 Years'} Exp</span></div>
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-heart w-4 md:w-5 text-cyan-400"></i><span>{talent.maritalStatus || 'Single'}</span></div>
                            <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-tag w-4 md:w-5 text-cyan-400"></i><span>{talent.workerType === 'Returned Housemaids' ? '🔄 Returned Housemaid' : '📋 Recruitment Worker'}</span></div>
                          </div>
                        </div>
                        <div className="flex gap-2 md:gap-3 mt-auto">
                          ...
                          <button onClick={() => handleHireClick(talent, 'Hire Talent')} className="flex-1 py-2 md:py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-center rounded-xl font-bold text-[8px] md:text-[10px] uppercase shadow-lg hover:shadow-xl transition-all">{t.hireBtn}</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <>
              <section id="home" className="relative pt-24 md:pt-32 pb-16 md:pb-32 px-4 md:px-6 qatar-gradient text-white overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none"><i className="fa-solid fa-globe text-[20rem] md:text-[40rem] absolute -top-20 -right-40 animate-spin-slow text-cyan-400"></i></div>
                <div className="absolute inset-0 opacity-20" style={{backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%2300ffff' fill-opacity='0.05'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")"}}></div>
                <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8 md:gap-16 items-center relative z-10">
                  <div className="space-y-6 md:space-y-8 animate-fade-in-up">
                    <span className="inline-block px-3 md:px-4 py-1 md:py-1.5 bg-cyan-500/20 backdrop-blur-md rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-widest border border-cyan-400/50 animate-pulse text-cyan-300">{t.certified}</span>
                    <h1 className="text-3xl md:text-7xl font-bold leading-[1.1]">{t.heroTitle} <span className="text-transparent bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text">{t.heroTitleSpan}</span> {t.heroTitleEnd}</h1>
                    <p className="text-sm md:text-lg text-gray-300 leading-relaxed max-w-lg">{t.heroDesc}</p>
                    <div className="flex flex-wrap gap-3 md:gap-4 justify-center md:justify-start">
                      <button onClick={() => handleQuickHire('House Maid')} className="group relative overflow-hidden bg-cyan-500/20 backdrop-blur-md border border-cyan-400/50 px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:bg-cyan-500 hover:text-white"><span className="relative z-10 flex items-center gap-1 md:gap-2 text-xs md:text-sm">🏠 {t.houseMaids}</span></button>
                      <button onClick={() => handleQuickHire('Driver')} className="group relative overflow-hidden bg-cyan-500/20 backdrop-blur-md border border-cyan-400/50 px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:bg-cyan-500 hover:text-white"><span className="relative z-10 flex items-center gap-1 md:gap-2 text-xs md:text-sm">🚗 {t.drivers}</span></button>
                      <button onClick={() => handleQuickHire('Nurse')} className="group relative overflow-hidden bg-cyan-500/20 backdrop-blur-md border border-cyan-400/50 px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:bg-cyan-500 hover:text-white"><span className="relative z-10 flex items-center gap-1 md:gap-2 text-xs md:text-sm">🏥 {t.nurses}</span></button>
                      <button onClick={() => handleExternalLink('https://alkhadam.net/qa/en/company/411', 'Monthly Cleaners')} className="group relative overflow-hidden bg-cyan-500/20 backdrop-blur-md border border-cyan-400/50 px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:bg-cyan-500 hover:text-white"><span className="relative z-10 flex items-center gap-1 md:gap-2 text-xs md:text-sm">🧹 {t.monthlyCleaners}</span></button>
                      <button onClick={() => handleExternalLink('https://alkhadam.net/qa/en/company/7653', 'Al-Mohannadi')} className="group relative overflow-hidden bg-cyan-500/20 backdrop-blur-md border border-cyan-400/50 px-4 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl font-bold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:bg-cyan-500 hover:text-white"><span className="relative z-10 flex items-center gap-1 md:gap-2 text-xs md:text-sm">🏥 {t.alMohannadi}</span></button>
                    </div>
                  </div>
                  <div className="hidden md:flex justify-center relative">
                    <div className="w-64 h-64 md:w-80 md:h-80 bg-cyan-500/10 backdrop-blur-xl border border-cyan-400/30 rounded-[3rem] md:rounded-[4rem] rotate-12 flex items-center justify-center shadow-2xl hover:rotate-0 transition-transform duration-700 hover:scale-105"><i className="fa-solid fa-building-columns text-[6rem] md:text-[10rem] opacity-20 text-cyan-400 -rotate-12"></i></div>
                    <div className="absolute -bottom-6 md:-bottom-10 -left-6 md:-left-10 p-4 md:p-8 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl md:rounded-3xl shadow-2xl text-white animate-float"><div className="text-2xl md:text-4xl font-bold">12+</div><div className="text-[8px] md:text-[10px] font-bold uppercase tracking-tighter leading-none">{t.yearsLabel}</div></div>
                  </div>
                </div>
              </section>

              <section className="py-12 md:py-16 bg-[#0a0a2a] border-b border-cyan-500/20 reveal">
                <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
                  {[{ num: '9.2K', label: t.successfulPlacements }, { num: '1.8K+', label: t.corporateClients }, { num: '24h', label: t.responseTime }, { num: '98.2%', label: t.complianceRate }].map((s, i) => (
                    <div key={i} className="flex items-center space-x-2 md:space-x-4 border-r border-cyan-500/20 last:border-0 hover:translate-x-2 transition-all duration-300 group">
                      <div className="text-2xl md:text-4xl text-cyan-400 font-black group-hover:scale-110 transition-transform">{s.num}</div>
              ...
              <section className="py-12 md:py-16 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a] px-4 md:px-6 reveal">
                <div className="max-w-7xl mx-auto">
                  <div className="flex justify-between items-center mb-6 md:mb-8 flex-wrap gap-4">
                    <h3 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{t.featuredCandidates}</h3>
                    <button onClick={() => setShowHirePage(true)} className="text-cyan-400 font-bold text-xs md:text-sm hover:underline transition-all flex items-center gap-1 group">{t.viewAllCandidates} <i className="fa-solid fa-arrow-right text-[10px] group-hover:translate-x-1 transition-transform"></i></button>
                  </div>
                  {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">{[...Array(6)].map((_, i) => <div key={i} className="bg-gray-800/50 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 animate-pulse"><div className="w-16 h-16 md:w-20 md:h-20 bg-gray-700 rounded-2xl mb-4"></div><div className="h-5 bg-gray-700 rounded w-3/4 mb-2"></div><div className="h-4 bg-gray-700 rounded w-1/2"></div></div>)}</div>
                        ...
                        <div key={talent.id} className="web3-card bg-gray-800/30 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm transition-all duration-500 hover:shadow-xl hover:-translate-y-2 flex flex-col h-full">
                          ...
                          <div className="flex-grow">
                            ...
                            <div className="mt-4 md:mt-6 pt-4 md:pt-6 border-t border-cyan-500/30 space-y-2 md:space-y-3 mb-6 md:mb-8">
                              ...
                              <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-user w-4 md:w-5 text-cyan-400"></i><span>{talent.gender}, {talent.age} Years</span></div>
                              <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-money-bill-wave w-4 md:w-5 text-cyan-400"></i><span>{talent.salary || 0} QAR</span></div>
                              <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-calendar-alt w-4 md:w-5 text-cyan-400"></i><span>{talent.experience || '3-5 Years'} Exp</span></div>
                              <div className="flex items-center text-xs text-gray-400"><i className="fa-solid fa-tag w-4 md:w-5 text-cyan-400"></i><span>{talent.workerType === 'Returned Housemaids' ? '🔄 Returned Housemaid' : '📋 Recruitment Worker'}</span></div>
                            </div>
                          </div>
                          <div className="flex gap-2 md:gap-3 mt-auto">
                            ...
                            <button onClick={() => handleHireClick(talent, 'Featured Hire')} className="flex-1 py-2 md:py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-center rounded-xl font-bold text-[8px] md:text-[10px] uppercase shadow-lg hover:shadow-xl transition-all">{t.hireBtn}</button>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <section id="about" className="py-16 md:py-24 px-4 md:px-6 bg-[#0a0a2a] reveal">
                <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-8 md:gap-16 items-center">
                  ...
                  <div className="space-y-6 md:space-y-8">
                    ...
                    <ul className="space-y-3 md:space-y-4">{[t.personalizedMatching, t.directLiaison, t.multiIndustry].map((item, i) => (<li key={i} className="flex items-center space-x-2 md:space-x-3 font-bold text-xs md:text-sm text-gray-300 group cursor-pointer"><i className="fa-solid fa-check-circle text-cyan-400 transition-transform group-hover:scale-110"></i><span className="group-hover:translate-x-1 transition-transform">{item}</span></li>))}</ul>
                    <button onClick={() => setShowAboutPage(true)} className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-6 md:px-8 py-2 md:py-3 rounded-lg md:rounded-xl font-bold text-xs md:text-sm hover:scale-105 transition-all shadow-md">{t.viewMore} <i className="fa-solid fa-arrow-right"></i></button>
                  </div>
                </div>
              </section>

              <section id="services" className="py-16 md:py-24 px-4 md:px-6 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a] reveal">
                <div className="max-w-7xl mx-auto text-center mb-12 md:mb-20"><h2 className="text-sm font-bold text-cyan-400 uppercase tracking-[0.3em] mb-4">{t.ourExpertise}</h2><h3 className="text-2xl md:text-4xl font-bold text-white">{t.comprehensiveSolutions}</h3></div>
                <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-6 md:gap-8">
                  {[{ icon: 'passport', title: t.visaTitle, desc: t.visaDesc }, { icon: 'users-gear', title: t.techTitle, desc: t.techDesc }, { icon: 'city', title: t.projectsTitle, desc: t.projectsDesc }].map((s, i) => (
                    <div key={i} className="bg-gray-800/30 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm hover:shadow-xl transition-all duration-500 group hover:-translate-y-3 border border-cyan-500/30 cursor-pointer">
                      <div className="w-12 h-12 md:w-16 md:h-16 bg-cyan-500/20 rounded-xl md:rounded-2xl flex items-center justify-center mb-6 md:mb-8 group-hover:bg-gradient-to-r group-hover:from-cyan-500 group-hover:to-blue-500 group-hover:text-white transition-all duration-500 group-hover:rotate-6"><i className={`fa-solid fa-${s.icon} text-xl md:text-2xl text-cyan-400`}></i></div>
                      <h4 className="text-base md:text-xl font-bold mb-3 md:mb-4 text-white group-hover:text-cyan-400 transition-colors">{s.title}</h4>
              ...
              <section className="py-12 md:py-16 px-4 md:px-6 bg-[#0a0a2a] reveal">
                <div className="max-w-7xl mx-auto">
                  <div className="text-center mb-6 md:mb-8">
                  ...
                  <div className="w-full h-[250px] md:h-[400px] rounded-xl md:rounded-2xl overflow-hidden shadow-xl border-2 border-cyan-500/30">
              ...
              <section className="py-16 md:py-20 bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a] px-4 md:px-6 reveal">
                <div className="max-w-7xl mx-auto">
                  ...
                  <div className="grid md:grid-cols-3 gap-6 md:gap-8">
                    {[{ text: t.testimonial1, author: t.author1 }, { text: t.testimonial2, author: t.author2 }, { text: t.testimonial3, author: t.author3 }].map((tst, i) => (
                      <div key={i} className="bg-gray-800/30 p-6 md:p-8 rounded-xl md:rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-2 border border-cyan-500/30">
                        <i className="fa-solid fa-quote-left text-2xl md:text-3xl text-cyan-400/30 mb-3 md:mb-4 block"></i>
                        <p className="text-gray-300 text-xs md:text-sm leading-relaxed mb-4 md:mb-6">{tst.text}</p>
                        <p className="font-bold text-cyan-400 text-xs md:text-sm">{tst.author}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <section className="py-16 md:py-20 bg-[#0a0a2a] px-4 md:px-6 reveal">
                <div className="max-w-5xl mx-auto">
                  ...
                  <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                    {[{ q: t.faqQ1, a: t.faqA1 }, { q: t.faqQ2, a: t.faqA2 }, { q: t.faqQ3, a: t.faqA3 }, { q: t.faqQ4, a: t.faqA4 }].map((faq, i) => (
                      <div key={i} className="bg-gray-800/30 p-4 md:p-6 rounded-xl md:rounded-2xl border border-cyan-500/30 shadow-sm faq-item cursor-pointer transition-all duration-300 hover:shadow-md" onClick={(e) => { const parent = e.currentTarget; parent.classList.toggle('active'); const answer = parent.querySelector('.faq-answer') as HTMLElement; if (parent.classList.contains('active')) answer.style.maxHeight = answer.scrollHeight + 'px'; else answer.style.maxHeight = '0px'; }}>
                        <h5 className="font-bold text-xs md:text-sm flex justify-between items-center uppercase tracking-tight text-white">{faq.q}<i className="fa-solid fa-plus text-[10px] md:text-xs transition-transform duration-300 ml-2 shrink-0 text-cyan-400"></i></h5>
              ...
              <section className="py-10 md:py-12 bg-gradient-to-r from-cyan-600 via-blue-600 to-cyan-600 text-white">
                <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-cyan-100 text-sm md:text-base">{t.comingSoonMsg}</p>
              ...
              <footer className="py-16 md:py-20 bg-[#0a0a2a] text-white px-4 md:px-6 border-t border-cyan-500/30">
                <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-8 md:gap-12 border-b border-cyan-500/20 pb-12 md:pb-16">
                  <div className="col-span-2">
                    <div className="flex items-center gap-3 mb-6">
                      <img src="/logo/logo.jpeg" alt="ZOD MANPOWER" className="w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover shadow-lg border border-cyan-400/30" />
                      <div className="text-lg md:text-2xl font-black uppercase tracking-tighter bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">{t.brandName}</div>
                    </div>
                    <p className="text-gray-500 text-xs md:text-sm leading-relaxed max-w-sm mb-6">{t.footerText}</p>
                    <div className="flex space-x-3 md:space-x-4"><a href="#" className="w-8 h-8 md:w-10 md:h-10 bg-cyan-500/20 rounded-full flex items-center justify-center hover:bg-cyan-500 transition-all duration-300 hover:scale-110"><i className="fa-brands fa-facebook-f text-sm md:text-base text-cyan-400"></i></a><a href="#" className="w-8 h-8 md:w-10 md:h-10 bg-cyan-500/20 rounded-full flex items-center justify-center hover:bg-cyan-500 transition-all duration-300 hover:scale-110"><i className="fa-brands fa-linkedin-in text-sm md:text-base text-cyan-400"></i></a></div>
                  </div>
                  <div><h6 className="font-bold uppercase text-[10px] md:text-xs tracking-widest mb-4 md:mb-6 text-cyan-400">{t.quickLinks}</h6><ul className="space-y-3 md:space-y-4 text-[10px] md:text-xs text-gray-500 font-bold uppercase"><li><a href="#about" className="hover:text-cyan-400 transition-all duration-300 hover:translate-x-1 inline-block">{t.aboutDoha}</a></li><li><a href="#services" className="hover:text-cyan-400 transition-all duration-300 hover:translate-x-1 inline-block">{t.clientServices}</a></li><li><button onClick={() => setShowHirePage(true)} className="hover:text-cyan-400 transition-all duration-300 hover:translate-x-1 inline-block">{t.browseCVs}</button></li></ul></div>
                  <div><h6 className="font-bold uppercase text-[10px] md:text-xs tracking-widest mb-4 md:mb-6 text-cyan-400">{t.internal}</h6><button onClick={() => setLoginModalOpen(true)} className="group flex items-center space-x-2 md:space-x-3 px-4 md:px-6 py-2 md:py-3 border border-cyan-500/30 rounded-xl md:rounded-2xl hover:bg-cyan-500 hover:text-[#0a0a2a] transition-all duration-300 hover:scale-105"><i className="fa-solid fa-lock text-[8px] md:text-[10px] group-hover:rotate-12 transition-transform text-cyan-400"></i><span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-cyan-400 group-hover:text-[#0a0a2a]">{t.adminPortal}</span></button></div>
                </div>
                <div className="max-w-7xl mx-auto pt-6 md:pt-8 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-6 text-[8px] md:text-[10px] text-gray-600 tracking-widest uppercase font-bold"><p>{t.copyright}</p><div className="flex space-x-4 md:space-x-6"><a href="#" className="hover:text-cyan-400 transition-colors">{t.privacyPolicy}</a><a href="#" className="hover:text-cyan-400 transition-colors">{t.terms}</a></div></div>
              </footer>
            </>
          )}
        </div>
      ) : (
        <div className="admin-section min-h-screen bg-gradient-to-br from-[#0a0a2a] to-[#1a1a4a] pb-16 md:pb-20">
          <nav className="bg-[#0a0a2a]/90 backdrop-blur-sm border-b border-cyan-500/30 px-4 md:px-6 py-3 md:py-4 mb-6 md:mb-10 sticky top-0 z-50">
            <div className="max-w-7xl mx-auto flex justify-between items-center">
              <div className="flex items-center space-x-2 md:space-x-3"><div className="w-6 h-6 md:w-8 md:h-8 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg flex items-center justify-center text-white"><i className="fa-solid fa-gears text-[8px] md:text-[10px]"></i></div><span className="font-bold text-xs md:text-sm tracking-widest uppercase text-white">{t.staffPortal}</span></div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-10">
              <div className="bg-gray-800/50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm hover:shadow-md transition-all duration-300"><p className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase tracking-widest mb-1 md:mb-2">{t.totalCandidates}</p><div className="text-2xl md:text-4xl font-bold text-white">{talents.length}</div></div>
              <div className="bg-gray-800/50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm hover:shadow-md transition-all duration-300"><p className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase tracking-widest mb-1 md:mb-2">{t.webLeads}</p><div className="text-2xl md:text-4xl font-bold text-indigo-400">{leads.length}</div></div>
              <div className="bg-gray-800/50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm hover:shadow-md transition-all duration-300"><p className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase tracking-widest mb-1 md:mb-2">{t.activeVacancies}</p><div className="text-2xl md:text-4xl font-bold text-white">0</div></div>
              <div className="bg-gray-800/50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/30 shadow-sm flex items-center justify-center"><button onClick={fetchTalents} className="bg-gradient-to-r from-cyan-500 to-blue-500 hover:shadow-lg text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-bold uppercase transition-all duration-300 hover:scale-105"><i className="fa-solid fa-rotate-right mr-1 md:mr-2"></i> {t.refresh}</button></div>
            </div>

            <div className="mb-6 md:mb-8">
              <div className="relative max-w-md">
                <i className="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-cyan-400 text-sm"></i>
                <input type="text" value={adminSearchQuery} onChange={(e) => setAdminSearchQuery(e.target.value)} placeholder={t.adminSearch} className="w-full p-4 pl-12 bg-gray-800 border border-cyan-500/30 rounded-2xl outline-none focus:ring-2 focus:ring-cyan-500 text-white transition-all text-sm" />
              </div>
              <p className="text-xs text-gray-500 mt-2 ml-2">{t.searchByName}</p>
            </div>

            <div className="flex space-x-4 md:space-x-6 mb-6 md:mb-8 border-b border-cyan-500/30">
              <button onClick={() => setActiveTab('candidates')} className={`pb-3 md:pb-4 border-b-2 font-bold text-[10px] md:text-xs uppercase tracking-widest transition-all duration-300 ${activeTab === 'candidates' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-gray-500 hover:text-gray-400'}`}>{t.inventoryManagement}</button>
              <button onClick={() => setActiveTab('leads')} className={`pb-3 md:pb-4 border-b-2 font-bold text-[10px] md:text-xs uppercase tracking-widest transition-all duration-300 ${activeTab === 'leads' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-gray-500 hover:text-gray-400'}`}>{t.visitorLogs}</button>
            </div>

            {activeTab === 'candidates' && (
              <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
                <div className="bg-gray-800/50 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-cyan-500/30 shadow-sm h-fit">
                  <div className="flex justify-between items-center mb-6 md:mb-8 border-b border-cyan-500/30 pb-3 md:pb-4"><h4 className="font-bold uppercase text-[10px] md:text-xs text-cyan-400 tracking-widest">{editTalent ? `${t.editCandidate} ${editTalent.name}` : t.newCandidate}</h4><button onClick={resetForm} className="text-[8px] md:text-[10px] text-gray-500 hover:text-red-400 transition-all hover:rotate-12"><i className="fa-solid fa-rotate-left"></i></button></div>
                  <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.fullName}</label><input ref={nameRef} type="text" className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all text-sm md:text-base" required /></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.dob}</label><input ref={dobRef} type="date" onChange={handleDobChange} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all" required /></div>
                    {calculatedAge !== null && (<div className="bg-cyan-500/20 p-2 md:p-3 rounded-lg md:rounded-xl"><span className="text-[10px] md:text-xs font-bold text-cyan-400">Age: {calculatedAge} years</span></div>)}
                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                      <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.gender}</label><select ref={genderRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all"><option>Male</option><option>Female</option></select></div>
                      <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.maritalStatus}</label><select ref={maritalStatusRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all">{maritalStatusOptions.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div>
                    </div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.jobDesignation}</label><select ref={jobRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all">{jobOptions.map(job => <option key={job} value={job}>{job}</option>)}</select></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.country}</label><select ref={countryRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all">{countryOptions.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.religion}</label><select ref={religionRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all"><option value="Muslim">Muslim</option><option value="Christian">Christian</option><option value="Hindu">Hindu</option><option value="Buddhist">Buddhist</option></select></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.salaryQAR}</label><input ref={salaryRef} type="number" defaultValue="0" step="100" className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all" required /></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.experience}</label><select ref={experienceRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all">{experienceOptions.map(exp => <option key={exp} value={exp}>{exp}</option>)}</select></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1">{t.workerType}</label><select ref={workerTypeRef} className="w-full p-3 md:p-4 bg-gray-900 border border-cyan-500/30 rounded-lg md:rounded-xl outline-none focus:bg-gray-800 focus:border-cyan-500 text-white transition-all">{workerTypeOptions.map(opt => <option key={opt} value={opt}>{opt === 'Recruitment Workers' ? t.recruitmentWorkers : t.returnedHousemaidsType}</option>)}</select></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1 block mb-1 md:mb-2">{t.photo}</label><input ref={picRef} type="file" accept="image/*" className="text-[10px] md:text-xs text-gray-400" /></div>
                    <div><label className="text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase ml-1 block mb-1 md:mb-2">{t.cvUpload}</label><input ref={cvRef} type="file" accept=".pdf,image/*" className="text-[10px] md:text-xs text-gray-400" /></div>
                    <button type="submit" disabled={isSubmitting} className="w-full py-3 md:py-4 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-lg md:rounded-xl font-bold uppercase text-[8px] md:text-[10px] tracking-widest shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 disabled:opacity-50">{isSubmitting ? <i className="fa-solid fa-spinner fa-spin mr-2"></i> : null}{t.saveRecord}</button>
                  </form>
                </div>
                <div className="lg:col-span-2 bg-gray-800/50 rounded-[2rem] md:rounded-[3rem] border border-cyan-500/30 shadow-sm overflow-x-auto">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[800px]">
                      <thead className="bg-gray-900/50 text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase tracking-widest border-b border-cyan-500/30">
                        ...
                        {adminFilteredTalents.map((talent) => (
                          <tr key={talent.id} className="hover:bg-gray-700/30 transition-all duration-200">
                            ...
                            <td className="p-4 md:p-6 text-right">
                              <button onClick={() => editHandler(talent)} className="text-cyan-400 p-1 md:p-2 hover:bg-cyan-500/20 rounded-lg transition-all duration-200 hover:scale-110"><i className="fa-solid fa-pen text-xs md:text-sm"></i></button>
            ...
            {activeTab === 'leads' && (
              <div className="bg-gray-800/50 rounded-[2rem] md:rounded-[3rem] border border-cyan-500/30 shadow-sm overflow-x-auto">
                <div className="p-4 md:p-8 border-b border-cyan-500/30 flex justify-between items-center flex-wrap gap-2"><h4 className="font-bold text-[10px] md:text-xs uppercase tracking-widest text-cyan-400">{t.realtimeLogs}</h4><button onClick={clearLeads} className="text-[8px] md:text-[10px] font-bold text-red-400 uppercase hover:underline transition-all">{t.clearLogs}</button></div>
                <table className="w-full text-left min-w-[400px]">
                  <thead className="bg-gray-900/50 text-[8px] md:text-[10px] font-bold text-cyan-400 uppercase tracking-widest">
                  ...
                  <tbody className="divide-y divide-cyan-500/20">
                    {leads.map((lead) => (<tr key={lead.id}><td className="p-4 md:p-8 text-[10px] md:text-xs font-bold text-gray-300">{lead.source}</td><td className="p-4 md:p-8 text-[10px] md:text-xs text-cyan-400 font-bold">{lead.action}</td><td className="p-4 md:p-8 text-right text-[8px] md:text-[10px] text-gray-500">{lead.time}</td></tr>))}
                  </tbody>
                </table>
              </div>
            )}
