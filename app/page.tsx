...
const translations = {
  en: {
    ...
    experience: 'Experience', driversJob: 'Drivers', babysitting: 'Baby sitting', nursesJob: 'Al-Mohannadi', cooks: 'Cook', domesticWorker: 'Domestic Worker', teacher: 'Teacher', houseMaid: 'House Maid',
  ...
  ar: {
    ...
    testimonial1: '"لقد أتيت إلى قطر عن طريق شركة زود لتوظيف القوى العاملة، وقد وجدوا لي مكاناً جيداً للعمل في الدوحة."',
    author1: '— محمد إكمار، سريلانكا',
    testimonial2: '"جئت من سريلانكا عبر شركة زود، وأعمل الآن في شركة رائدة."',
    author2: '— محمد ر.',
    testimonial3: '"وفرت لنا شركة زود للقوى العاملة أكثر من 50 موظفاً لفندقنا"',
    author3: '— فاطمة سارة، مديرة',
    ...
    experience: 'الخبرة', driversJob: 'سائقين', babysitting: 'رعاية أطفال', nursesJob: 'المهندي', cooks: 'طهاة', domesticWorker: 'عمال منازل', teacher: 'معلمين', houseMaid: 'خادمة منزل',
            ...
            {activeTab === 'leads' && (
              <div className="bg-white rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-sm overflow-x-auto">
                ...
                <table className="w-full text-left min-w-[400px]">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {leads.map((lead) => (<tr key={lead.id}><td className="p-4 md:p-8 text-[10px] md:text-xs font-bold">{lead.source}</td><td className="p-4 md:p-8 text-[10px] md:text-xs text-indigo-600 font-bold">{lead.action}</td><td className="p-4 md:p-8 text-right text-[8px] md:text-[10px] text-gray-400">{lead.time}</td>))}
                  </tbody>
                </table>
              </div>
            )}
